...
import { commentPosition, CommentBaseParam, commentSizeString, commentOption, fontSize, commentLinesBySize } from './types'
...
export class CommentBase {
    ...
    private customAttr: Map<string, any>;//カスタム属性
     ...
     */
    private _getLines(size: commentSizeString, allLines: commentLinesBySize): number {
        switch (size) {
            case 'big':
                return allLines.big;
            case 'medium':
     ...
     * @param type コメントの位置
     */
    private _sortByType(comments: Array<string>, type: commentPosition): Array<string> {
    ...
    clear(option?: 'first' | 'last') {
        switch (option) {
            ...
            case 'first':
                ...
                break;
            case 'last':
                if (this.comments.length) this.comments[this.comments.length - 1].kill();
        }
    }
